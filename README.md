...
- 在window中编译指令
```bash
 hvigorw  clean --mode module -p product=default -p buildMode=debug assembleHap --analyze=normal --no-daemon --no-parallel
```
- 在mac中使用编译指令

```bash
hvigorw  clean --mode module -p product=mac_debug -p buildMode=debug assembleHap --analyze=normal --no-daemon --no-parallel
```
